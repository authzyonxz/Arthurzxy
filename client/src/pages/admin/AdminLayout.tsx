...
import { LayoutDashboard, Users, Package, Trophy, History, LogOut, Zap, Menu, X } from "lucide-react";
import { useState } from "react";
import { toast } from "sonner";

const navItems = [
  { href: "/admin/dashboard", icon: LayoutDashboard, label: "Dashboard" },
  { href: "/admin/resellers", icon: Users, label: "Revendedores" },
  { href: "/admin/stock", icon: Package, label: "Estoque Geral" },
...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  const { session, isLoading, isAdmin, logout } = useCustomAuth();
    ...
    <div className="min-h-screen grid-bg flex">
      ...
      `} style={{ background: "oklch(0.09 0.025 240)", borderRight: "1px solid oklch(0.2 0.04 240)" }}>
        ...
        <div className="p-5 border-b" style={{ borderColor: "oklch(0.2 0.04 240)" }}>
          ...
          <div className="mt-1 text-xs tracking-widest uppercase" style={{ color: "oklch(0.65 0.2 160)", fontFamily: "'Share Tech Mono', monospace" }}>
            ADMINISTRADOR
          </div>
        </div>
          ...
          {navItems.map(({ href, icon: Icon, label }) => {
            ...
            return (
              <Link key={href} href={href} onClick={() => setSidebarOpen(false)}>
                <div className={`flex items-center gap-3 px-3 py-2.5 rounded text-sm font-medium tracking-wide cursor-pointer transition-all ${isActive ? "sidebar-active" : ""}`}
                  style={{
